...
export default function Profile() {
  ...
  // // Fix: Properly filter activities based on current filter
  // const filteredActivities = activityHistory.filter(a =>
  ...
  const tabContent = {
    ...
    //     <div className="flex items-center gap-1 px-4 mb-6 bg-gray-50 rounded-2xl p-1 border border-gray-100">
    //       {[
    //         { key: "all", label: "All", icon: Activity },
    //         { key: "review", label: "Reviews", icon: Star },
    //         { key: "support", label: "Support", icon: Coins },
    //         { key: "verify", label: "Verifications", icon: CheckCircleSolid },
    //       ].map((f) => {
    ...
    //             className={`flex items-center justify-center gap-1.5 text-sm px-3 py-2 rounded-xl font-medium transition-all duration-200 flex-1 ${activityFilter === f.key
    ...
    //           <div className="w-20 h-20 rounded-2xl bg-white border border-gray-200 flex items-center justify-center mb-5">
    //             <Activity className="w-10 h-10 text-gray-400" strokeWidth={1.5} />
    //           </div>
    //           <h3 className="font-medium text-gray-600 mb-2">No activity yet</h3>
    //           <p className="text-sm text-gray-500 text-center max-w-xs">
    //             {activityFilter === "all"
    //               ? "Start reviewing movies or supporting creators to see activity here."
    //               : activityFilter === "review"
    //                 ? "You haven't reviewed any movies yet."
    //                 : activityFilter === "support"
    //                   ? "You haven't supported any reviews yet."
    ...
    //               }}
    //               className="bg-white rounded-2xl p-4 flex items-start gap-4 cursor-pointer hover:shadow-md transition-all duration-200 border border-gray-100 active:scale-[0.98]"
    //             >
    //               <div className={`w-12 h-12 rounded-xl overflow-hidden flex items-center justify-center flex-shrink-0 ${item.type === "review" ? "bg-yellow-50" :
    //                 item.type === "support" ? "bg-green-50" :
    //                   "bg-blue-50"
    //                 }`}>
    //                 {item.type === "review" && <Star className="w-6 h-6 text-yellow-500" strokeWidth={1.5} />}
    //                 {item.type === "support" && <Coins className="w-6 h-6 text-green-500" />}
    //                 {item.type === "verify" && <CheckCircleSolid className="w-6 h-6 text-blue-500" />}
    //               </div>

    //               <div className="flex-1 min-w-0">
    //                 <div className="flex items-start justify-between mb-1">
    //                   <div className="text-sm font-semibold text-gray-900 pr-2">{item.title}</div>
    ...
    //                 {item.posterUrl && (
    //                   <div className="mt-3 w-16 h-24 rounded-md overflow-hidden bg-gray-200 border border-gray-300">
    //                     <Image
    //                       src={item.posterUrl}
    //                       alt=""
    //                       width={64}
